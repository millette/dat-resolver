...
class DatLru extends AsyncLRU {
  constructor (path, opts) {
    ...
    super({ max, load: DatLru.load(path, opts, mkdirError) })
    this.ready = null
    this.error = null
    this.get = util.promisify(AsyncLRU.prototype.get.bind(this))

    this.on('evict', (o) => {
      // console.log('EVICTING', new Date().toISOString())
      DatLru.cleanDat(o.value)
        .then(() => {
          this.emit('dat-evict', o) // emit('evict') creates an infinite loop
        })
        .catch((e) => {
          console.log('EVICT err:', e)
        })
    })
  ...
  cleanup () {
    // console.log('CLEANUP:', this && this.keys && this.keys.length && this.keys)
    if (!this.keys.length) { return Promise.resolve('ok-nothing') }
    const ps = []
    this.keys.forEach((k) => { ps.push(this.get(k).then(DatLru.cleanDat)) })
