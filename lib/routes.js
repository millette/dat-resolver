...
const version = (ctx, key) => ctx.datLru.get(key)
  .then((dat) => {
    ctx.type = 'application/json'
    ctx.body = { version: dat.version }
  })
  .catch((e) => ctx.throw(404, e))

const peers = (ctx, key) => ctx.datLru.get(key)
  .then((dat) => {
    ctx.type = 'application/json'
    ctx.body = { peers: pick(dat.network, ['connected', 'connecting']) }
  })
  .catch((e) => ctx.throw(404, e))
...
module.exports = { top, datKeyM, home, version, peers }
